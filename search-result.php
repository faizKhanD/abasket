...
if(isset($_REQUEST['btn-ser']))
{
$ser_key=$_POST['search'];

$sql1="SELECT product.*, option_detail.sale_price as op_sale, option_detail.price as op_price, option_value.value as opt_val from product inner join option_detail on product.id=option_detail.p_id left join option_value on option_detail.value_id=option_value.id WHERE `name` LIKE '%$ser_key%'";
}
